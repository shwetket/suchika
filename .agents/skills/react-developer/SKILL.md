...
- Follow the latest UCX design system and UI improvements.
- Routinely remove unused components and files to keep the frontend lean.
...
- Test framework: Jest + React Testing Library (already configured). Playwright for E2E.
- Cover: happy path render, error state, loading state, role-based access. Form fields must have `htmlFor`/`id` pairings in shared components (e.g., `Field`) for Playwright `getByLabel()`.
...
- Add comprehensive unit tests covering all components and various edge cases (following the pattern established in the wealth domain).
