...
- Discriminator columns stay VARCHAR — no SQL ENUMs, no CHECK constraints for business rules or enums.
...
- Drive contract consolidation across all domains (e.g., unified error responses and shared pagination contracts as seen in Q54). Ensure duplicated API shapes are extracted to the `shared` module.
...
- Adapters must be testable with Testcontainers + `%integration-test` profile and `RestAssured` (real PostgreSQL, no direct Java calls).
