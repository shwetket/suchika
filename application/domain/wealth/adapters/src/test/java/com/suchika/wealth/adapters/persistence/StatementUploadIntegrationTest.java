...
class StatementUploadIntegrationTest {
    ...
    // Seeded in R__seed_wealth_test_data.sql — SAVINGS account for the test profile
    ...
    void uploadStatement_parsesAndPersistsAllRows() {
        StatementUpload upload = uploadUseCase.uploadStatement(SEEDED_ACCOUNT_ID, "june.csv", HDFC_SAVINGS_CSV);

        assertEquals(UploadStatus.SUCCESS, upload.getStatus());
        assertEquals("june.csv", upload.getFileName());
        assertEquals(SEEDED_ACCOUNT_ID, upload.getAccountId());
    ...
    void uploadStatement_transactionsLinkedToUpload() {
        StatementUpload upload = uploadUseCase.uploadStatement(SEEDED_ACCOUNT_ID, "link-test.csv", HDFC_SAVINGS_CSV);

        List<Transaction> txns = transactionRepository.findByAccountId(SEEDED_ACCOUNT_ID, null, null, null);
        assertTrue(txns.stream().allMatch(t -> upload.getId().equals(t.getUploadId())));
    }
    ...
    void uploadStatement_crossFileDuplicate_secondUploadRowSkipped() {
        ...
        // First upload: row is new, should be saved
        ...
        // Second upload with same row: should be silently skipped (cross-file dedup)
        StatementUpload second = uploadUseCase.uploadStatement(SEEDED_ACCOUNT_ID, "second.csv", singleRowCsv);
        assertEquals(UploadStatus.SUCCESS, second.getStatus());

        List<Transaction> afterSecond = transactionRepository.findByAccountId(SEEDED_ACCOUNT_ID,
                LocalDate.of(2026, Month.JUNE, 10), LocalDate.of(2026, Month.JUNE, 10), null);
        assertEquals(1, afterSecond.size()); // still only 1 — duplicate was dropped
    }

    @Test
    void rollbackUpload_cascadeDeletesAllTransactions() {
        StatementUpload upload = uploadUseCase.uploadStatement(SEEDED_ACCOUNT_ID, "to-rollback.csv", HDFC_SAVINGS_CSV);
        ...
        // After rollback, all transactions from this upload should be gone
    ...
    void listUploads_returnsInDateDescOrder() {
        ...
        // All uploads we created should be there (we filter by SEEDED_ACCOUNT_ID)
