...
- **Application Console — design only (Phase 4), see ADR-023:** admin-only page for live service status, start/stop controls, and per-service error stats/log viewer. Introduces two new architectural surfaces, both flag-gated (`suchika.console.enabled=false` by default): (1) `com.suchika.gateway.console` package in `web-gateway` — `ServiceControlService` shells out to `scripts/run-local.ps1`/`.sh` and `stop-local.ps1`/`.sh` using a new, parallel-workstream `-Service <name>` per-service argument (not the whole-stack form); `ServiceStatusService` polls each service's `/q/health` (Phase 1) and reads the PID registry at `~/.suchika/run/<service>.pid` (Phase 1). This is a deliberate, scoped exception to the gateway's side-effect-free role (ADR-002/ADR-013), justified the same way ADR-021 justified auto-attach-to-sole-admin — flagged for mandatory re-examination once real OIDC auth lands at v1.0 (ADR-005). (2) A new `error_log` table per domain (own schema, own Flyway migration — no shared cross-domain table, per ADR-003/ADR-006), each exposing `GET /v1/errors?since=&limit=`; `web-gateway` adds `GET /v1/console/errors` fanning out over REST, mirroring ADR-013's aggregation pattern. Not yet implemented; open questions (error_log scoping, ROADMAP v1.0 checklist entry, retention policy) flagged in ADR-023 for product-owner decision.
