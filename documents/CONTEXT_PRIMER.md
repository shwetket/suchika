...
- **Application Console — backend + frontend implemented (Phase 4), see ADR-023:** admin-only live service status, start/stop controls, and a per-domain error feed. Two architectural surfaces, both flag-gated (`suchika.console.enabled=false` by default in `application.properties` — every `ConsoleResource` endpoint 404s while off, verified by a dedicated `ConsoleResourceDisabledTest` that deliberately does not override the flag; `ConsoleResourceTest` covers the enabled behavior via a `@TestProfile` config override).
  ...
  - **Revised same day (2026-07-13): the per-domain error-log vertical slice above was 4x byte-for-byte duplicated code (52 files, identical except package name) — flagged in product-owner review.** ADR-023 amended in place: `ErrorLog` (domain)/`ErrorLogUseCase`/`ErrorLogRepository` (ports) move into the existing `shared/` module (zero new dependency — they're already plain Java); the JAX-RS resource logic and Panache repository/service query logic move into a **new leaf module, `shared-adapter`** (depends on `shared`; depended on only by each domain's own `adapters` module, never by `domain`/`ports`), as abstract base classes each domain subclasses with a ~10-20 line concrete binding. Each domain still owns its own schema/table/Flyway migration/`@Entity` class — no schema change. Also fixes a real info-exposure gap: the shared `ErrorLogResponse` mapping now truncates/sanitizes `details` before it reaches the wire (endpoints are still unauthenticated per ADR-005, unchanged). See ADR-023's "Revision — 2026-07-13" section for full reasoning (why not centralize in `profile`, why a new module instead of adding to `shared/` directly, why an abstract-base-class strategy instead of relying on unverified cross-module Quarkus/Jandex discovery). Not yet implemented as of this note — design only.
