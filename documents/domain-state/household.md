...
- 🔲 **FLAG (2026-07-11, doc-consolidation pass): `application/flyway/test-seed/household/R__seed_household_test_data.sql` seeds one real calendar event + one real inventory row against a real profile_id, and its own header claims it is gitignored/branch-isolated — it currently is not.** Same underlying issue as `documents/domain-state/profile.md`'s Open Issues flag (see there for the full explanation): `.gitignore` has no `test-seed` entry on this branch and the file is tracked/committed. Household's own exposure here is small (2 rows, no financial data), but it's part of the same seed-data set and should be resolved together with the profile/wealth/health files, not independently. Not fixed here.
