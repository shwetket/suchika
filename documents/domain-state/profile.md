...
- Profile avatar/photo (v0.4 or later) — candidate consumer of the currently-dead `metadata` column below, if picked up.
- Admin authentication (v1.0)
- 🔲 **New (2026-07-06 retrospective): `profile.profile.metadata JSONB` is a dead column.** Present in `V1__init_profile_consolidated.sql` (`NOT NULL DEFAULT '{}'::jsonb`) but not mapped in `ProfileEntity`, not in `application/contract/profile.yaml`, not in any DTO or service code — zero consumers anywhere in the domain. Looks like a speculative addition (mirroring the `wealth.account.metadata` pattern) that was never wired up. Since this is pre-release, either give it a real purpose (e.g. the avatar/photo backlog item) or drop it in a new Flyway migration — don't leave unused JSONB columns on the identity-anchor table.
